const App = (() => {
  ...
  const elements = {
    ...
    calendarEventBox: document.getElementById("calendarEventBox"),
  ...
  async function handleAuthSignIn() {
    ...
    if (!email || !password) {
      alert("Wpisz email i has?o.");
      return;
    }
    const { error } = await authClient.auth.signInWithPassword({ email, password });
    if (error) {
      alert("Nie uda?o si? zalogowa?. Sprawd? dane logowania.");
      return;
    }
  ...
  async function handleAuthRegister() {
    ...
    if (password.length < 6) {
      alert("Has?o musi mie? minimum 6 znak?w.");
      return;
    }
    ...
    if (error) {
      alert("Nie uda?o si? utworzy? konta.");
      return;
    }
  ...
  function bindEvents() {
    ...
    if (elements.calendarAddBtn) {
      elements.calendarAddBtn.addEventListener("click", openCalendarEventBox);
    }
    ...
    if (elements.calendarEventCancelBtn) {
      elements.calendarEventCancelBtn.addEventListener("click", closeCalendarEventBox);
    }
  ...
  function initCalendarEventForm() {
    if (elements.calendarEventBox) {
      elements.calendarEventBox.hidden = true;
    }
  ...
  function openCalendarEventBox() {
    if (!elements.calendarEventBox) {
      return;
    }
    elements.calendarEventBox.hidden = false;
  ...
  function closeCalendarEventBox() {
    if (!elements.calendarEventBox) {
      return;
    }
    elements.calendarEventBox.hidden = true;
  ...
  function updateTimerControls() {
    ...
    elements.timerPauseBtn.textContent = timerState.pauseStartedAt ? "Wzn?w" : "Pauza";
  }
  ...
  function isPolishHoliday(iso) {
    ...
    return iso === easterIso || iso === easterMondayIso || iso === corpusChristiIso;
  }
  ...
  function renderMonthCalendar() {
    ...
    for (let i = 0; i < 42; i += 1) {
      ...
      const title = isWorked
        ...
        : isVacation
          ...
          : isSickDay
            ? `${formatDate(iso)} | L4`
          : isManualOffDay
            ? `${formatDate(iso)} | Dzie? wolny`
            : isHoliday
            ? `${formatDate(iso)} | Dzie? wolny`
            : isFuture
              ? `${formatDate(iso)} | Dzie? przysz?y`
      ...
      cells.push(`<div class="month-calendar__cell ${statusClass}${todayClass}${outsideClass}" title="${title}">${currentDate.getDate()}</div>`);
    }
          ...
          (entry) => `
            <li class="entries-v3-item" data-entry-id="${entry.id}">
              ...
              <div class="entries-v3-side">
                ...
                <button type="button" class="entries-v3-remove" data-id="${entry.id}">Usun</button>
              </div>
            </li>
          `
      )
